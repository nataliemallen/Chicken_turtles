...
pop<-read.csv("/Users/natal/R/turtles/ct_nobo_pop.csv", header = T)
...
# Read the kinship data
kin <- read.csv("ct_kin_new.csv", header = TRUE)

# Define kinship categories and bounds
bounds <- matrix(c(0.423128, Inf, 0.1767767, 0.3535534,   # Parent Offspring
                   0.2, 0.5, 0.08838835, 0.1767767,       # Half Sib
                   0.07142857, 0.4210526, 0.04419417, 0.08838835,  # First cousins
                   -Inf, Inf, -Inf, 0.04419417),          # Unrelated
...
colors <- c("#E76BF3", "#00A5FF", "#00BC59", "#F8766D")
labels <- c("1st degree", "2nd degree", "3rd degree", "Unrelated")

# Function to create pie chart for a specific population
create_pie_chart <- function(population_data, population_name) {
  # Calculate counts for each kinship category
  counts <- rep(0, length(colors))
  for (i in 1:length(colors)) {
    counts[i] <- sum(population_data$R1 >= bounds[i, 1] & population_data$R1 <= bounds[i, 2] &
                       population_data$KING >= bounds[i, 3] & population_data$KING <= bounds[i, 4])
  }
  
  # Plot pie chart
  pie(counts, labels = labels, col = colors, main = population_name)
}

# List of populations and their data frames
populations <- list(Buller = Buller, Warren = Warren, Alazan = Alazan, Brazoria = Brazoria, Gordy = Gordy, Wharton = Wharton)

# Create pie charts for each population
for (pop_name in names(populations)) {
  population_data <- populations[[pop_name]]
  create_pie_chart(population_data, pop_name)
}
